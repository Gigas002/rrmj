...
// --- win scoring fixtures ---

/// Table-driven win case for baseline yaku verification for standard yaku scoring.
#[derive(Debug, Clone)]
struct WinCase {
    id: &'static str,
    concealed: Vec<Tile>,
    ...
    wall_live_remaining: Option<usize>,
    is_rinshan_draw: bool,
    is_dealer_first_turn: bool,
    live_draws: [u8; 4],
    calls_made: bool,
    is_chankan: bool,
...
/// Win fixtures for Phase 11.4 baseline yaku for standard yaku scoring.
fn baseline_cases() -> Vec<WinCase> {
    vec![
        case(
            "menzen_tsumo_tanyao",
            &[
...
fn riichi_timing_cases() -> Vec<WinCase> {
    vec![
        case(
...
fn state_for_case(case: &WinCase) -> HandState {
    let config = RulesConfig::standard();
    ...
    if case.id.starts_with("renhou_") {
...
fn win_timing_cases() -> Vec<WinCase> {
    vec![
        WinCase {
...
fn score_case(case: &WinCase) -> crate::scoring::ScoringResult {
    let config = RulesConfig::standard();
    let profile = RulesRegistry::get(RulesProfileId::Standard).unwrap();
...
#[test]
fn recommendations_match_score_win_on_fixtures() {
    let config = RulesConfig::standard();
    for case in all_win_cases() {
        ...
        let state = state_for_case(&case);
        let paths = crate::rules::recommendations(&state, case.winner, &config, 8);
        assert!(
            !paths.is_empty(),
